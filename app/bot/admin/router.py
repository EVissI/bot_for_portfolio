...
@admin_router.message(F.text.lower() == "пропустить", StateFilter(AddProject.name))
async def process_name(message: Message, state: FSMContext):
    await message.answer(
        "Этот параметр обязательный, без него дальше незя,\n Введите название проекта:"
    )
...
@admin_router.message(StateFilter(AddProject.small_deskript))
...
@admin_router.message(StateFilter(AddProject.large_deskript))
...
@admin_router.message(
    F.text.lower == "пропустить" or F.text.regexp(telegram_bot_link_pattern),
...
@admin_router.message(
    F.text.lower == "пропустить" or F.text.regexp(https_link_pattern),
...
async def process_github_link(message: Message, state: FSMContext):
    ...
    await message.answer("Подтвердите публикацию")
