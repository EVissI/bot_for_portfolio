...
from bot.keyboard.markup_kb import cancel_button,MainKeyboard
...
async def add_new_project(message: Message, state: FSMContext):
    ...
    await message.answer("Введите название проекта:", reply_markup=cancel_button('create'))


@create_project.message(StateFilter(AddProject) and F.text.lower() == "отменить создание" )
...
async def process_name(message: Message, state: FSMContext,session,**kwargs):
    ...
    await message.answer(
        f"Введите краткое описание проекта", reply_markup=cancel_button('create')
    )


@create_project.message(StateFilter(AddProject.small_deskript))
async def process_small_deskript(message: Message, state: FSMContext):
    ...
    await message.answer(
        f"Введите полное описание проекта", reply_markup=cancel_button('create')
    )


@create_project.message(StateFilter(AddProject.large_deskript))
async def process_large_deskript(message: Message, state: FSMContext):
    ...
    await message.answer(
        f"Введите тег бота", reply_markup=cancel_button('create')
    )
...
async def process_telegram_bot_link(message: Message, state: FSMContext):
    ...
    await message.answer(
        f"Введите ссылку на проект", reply_markup=cancel_button('create')
    )
...
@create_project.callback_query(AdminCallbackAddProject.filter())
@connection()
...
@create_project.callback_query(AdminCallbackAddProjectChange.filter())
async def process_change_qr(query: CallbackQuery, callback_data: AdminCallbackAddProject,state:FSMContext):
    await query.message.delete()
    await query.message.answer('Теперь введите заменяющее значение',reply_markup=cancel_button('create'))
